import { Injectable, ConflictException } from '@nestjs/common';
import { PrismaService } from 'src/shared/services/prisma.service';
import { Author, Prisma } from '@prisma/client';

@Injectable()
export class AuthorsService {
    constructor(private prismaService: PrismaService) {}

    public async getAll(): Promise<Author[]> {
        return this.prismaService.author.findMany();
    }

    public async getById(id: string): Promise<Author | null> {
        return this.prismaService.author.findUnique({
            where: {id},
        });
    }

    public async create(authorData: Prisma.AuthorCreateInput): Promise<Author> {
        try {
            return await this.prismaService.author.create({
                data: authorData,
            });
        } catch (error) {
            if (error.code === 'P2002') {
                throw new ConflictException('Name is already taken');
            }
            throw error;
        }
    }

    public async updateById(id: string, authorData: Prisma.AuthorUpdateInput): Promise<Author> {
        return this.prismaService.author.update({
            where: {id},
            data: authorData,
        });
    }

    public async deleteById(id: string): Promise<Author> {
        return this.prismaService.author.delete({
            where: {id},
        });
    }
}
